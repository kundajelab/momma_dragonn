...
densityMotifSimulation.py --prefix gata --motifNames GATA_disc1 --max-motifs 3 --min-motifs 0 --mean-motifs 1 --seqLength 200 --numSeqs 1000
densityMotifSimulation.py --prefix tal --motifNames TAL1_known1 --max-motifs 3 --min-motifs 0 --mean-motifs 1 --seqLength 200 --numSeqs 1000
densityMotifSimulation.py --prefix talgata --motifNames GATA_disc1 TAL1_known1 --max-motifs 3 --min-motifs 0 --mean-motifs 1 --seqLength 200 --numSeqs 1000
...
zcat DensityEmbedding_prefix-gata_motifs-GATA_disc1_*_seqLength-200_numSeqs-1000.simdata.gz | perl -lane 'if ($. > 1) {print "$F[0]\t1\t0"}' > labels_without_title.txt
zcat DensityEmbedding_prefix-tal_motifs-TAL1_known1_*_seqLength-200_numSeqs-1000.simdata.gz | perl -lane 'if ($. > 1) {print "$F[0]\t0\t1"}' >> labels_without_title.txt
zcat DensityEmbedding_prefix-talgata_motifs-GATA_disc1+TAL1_known1_*_seqLength-200_numSeqs-1000.simdata.gz | perl -lane 'if ($. > 1) {print "$F[0]\t1\t1"}' >> labels_without_title.txt
